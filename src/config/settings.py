...
def load_config(path = "config/config.yaml"):
    ...
    return Config(
        ...
        workstationconfig=WorkStationConfig(
            ...
            gripper_count=workstation_data.get("gripper_count", WorkStationConfig.gripper_count),
            cabinet_layers=workstation_data.get("cabinet_layers", WorkStationConfig.cabinet_layers),
            max_stacks_per_layer=workstation_data.get("max_stacks_per_layer", WorkStationConfig.max_stacks_per_layer),
        ),
        conveyorconfig=ConveyorConfig(
            cargo_min_spacing=conveyor_data.get("cargo_min_spacing", ConveyorConfig.cargo_min_spacing),
            speed=conveyor_data.get("speed", ConveyorConfig.speed),
        ),
        scannerconfig=ScannerConfig(
            scanner_timeout=scanner_data.get("scanner_timeout", ScannerConfig.scanner_timeout),
        ),
        exceptionconfig=ExceptionConfig(
            max_retry_count=exception_data.get("max_retry_count", ExceptionConfig.max_retry_count),
        )
    )
