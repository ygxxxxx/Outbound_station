...
# RCS通信模块（作为服务端）
class RCS_Sever:
    def __init__(self, status_host: str, status_port: int, task_host: str, task_port: int) -> None:
        ...
        try:
            while not self.status_stop_event.is_set():
                ...
                while True:
                    ...
                    self._status_handle_message(seq, cmd, body_dict)
        ...
        try:
            while not self.task_stop_event.is_set():
                ...
                while True:
                    ...
                    seq, cmd, body_dict = result
                    logger.info(f"[接收] seq = {seq}, cmd = {cmd}, body_dict = {body_dict}")
                    self._task_handle_message(seq, cmd, body_dict)
        except Exception as e:
            logger.error(f"接收数据时发生错误:{e}")
        finally:
            conn.close()
